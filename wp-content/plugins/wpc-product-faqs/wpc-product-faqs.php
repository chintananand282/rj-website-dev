...
! defined( 'WPCPF_VERSION' ) && define( 'WPCPF_VERSION', '2.1.1' );
		...
		if ( ! class_exists( 'WPCleverWpcpf' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcpf {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WPCPF_FILE );
						}
					} );
				}
					...
					<table class="wpcpf_configuration_table">
						...
						<tr class="wpcpf_configuration_tr">
							<td class="wpcpf_configuration_th">
								<select name="wpcpf_type" class="wpcpf_type">
				...
				function action_links( $links, $file ) {
					...
					if ( $plugin === $file ) {
						$how = '<a href="' . admin_url( 'admin.php?page=wpclever-wpcpf&tab=how' ) . '">' . esc_html__( 'How to use?', 'wpc-product-faqs' ) . '</a>';
				...
				function admin_menu_content() {
					...
					<div class="wpclever_settings_page wrap">
						<h1 class="wpclever_settings_page_title"><?php echo esc_html__( 'WPC Product FAQs', 'wpc-product-faqs' ) . ' ' . WPCPF_VERSION; ?></h1>
						<div class="wpclever_settings_page_desc about-text">
							<p>
								<?php printf( esc_html__( 'Thank you for using our plugin! If you are satisfied, please reward it a full five-star %s rating.', 'wpc-product-faqs' ), '<span style="color:#ffb900">&#9733;&#9733;&#9733;&#9733;&#9733;</span>' ); ?>
								<br/>
						...
						<div class="wpclever_settings_page_nav">
							<h2 class="nav-tab-wrapper">
								<a href="<?php echo admin_url( 'admin.php?page=wpclever-wpcpf&tab=how' ); ?>" class="<?php echo esc_attr( $active_tab === 'how' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'How to use?', 'wpc-product-faqs' ); ?>
								</a>
								<a href="<?php echo admin_url( 'edit.php?post_type=wpc_product_faq' ); ?>" class="nav-tab">
				...
				function ajax_add_faq() {
					$type = isset( $_POST['type'] ) ? sanitize_key( $_POST['type'] ) : 'custom';

					self::faq( [
						'type'    => $type,
						'title'   => esc_html__( 'FAQ title', 'wpc-product-faqs' ),
						'editor'  => isset( $_POST['editor'] ) ? sanitize_key( $_POST['editor'] ) : '',
						'content' => ''
					], true );
				...
				function faq( $faq, $new = false ) {
					...
					<div class="wpcpf-faq">
						...
						<?php
						if ( $faq['type'] === 'custom' ) {
							$editor_id = ! empty( $faq['editor'] ) ? $faq['editor'] : uniqid( 'wpcpf-editor-', false ); ?>
							<div class="wpcpf-faq-line">
								<input type="hidden" name="wpcpf_faqs[<?php echo esc_attr( $key ); ?>][type]" value="<?php echo esc_attr( $faq['type'] ); ?>"/>
								<input type="text" name="wpcpf_faqs[<?php echo esc_attr( $key ); ?>][title]" placeholder="<?php esc_attr_e( 'FAQ title', 'wpc-product-faqs' ); ?>" style="width: 100%" value="<?php echo esc_attr( $faq['title'] ); ?>" required/>
							</div>
							<div class="wpcpf-faq-line">
								<?php
								if ( $new ) {
									echo '<textarea id="' . $editor_id . '" name="wpcpf_faqs[' . esc_attr( $key ) . '][content]" rows="10"></textarea>';
								} else {
									wp_editor( $faq['content'], $editor_id, [
										'textarea_name' => 'wpcpf_faqs[' . esc_attr( $key ) . '][content]',
										'textarea_rows' => 10
									] );
								}
								?>
						...
						<?php } else { ?>
							<div class="wpcpf-faq-line">
								<input type="hidden" name="wpcpf_faqs[<?php echo esc_attr( $key ); ?>][type]" value="<?php echo esc_attr( $faq['type'] ); ?>"/>
								<select class="wpcpf-faq-search" multiple="multiple" name="wpcpf_faqs[<?php echo esc_attr( $key ); ?>][title][]">
									<?php
									...
									if ( ! empty( $selected_faqs ) ) {
										foreach ( $selected_faqs as $selected_faq ) {
											if ( ! empty( $selected_faq ) && ( $selected_faq_data = get_post( $selected_faq ) ) ) {
												echo '<option value="' . $selected_faq_data->ID . '" selected>' . $selected_faq_data->post_title . '</option>';
											}
										}
									}
									?>
								</select>
								<input type="hidden" name="wpcpf_faqs[<?php echo esc_attr( $key ); ?>][content]" value="auto"/>
							</div>
						<?php } ?>
					</div>
					<?php
				...
				function new_faq() {
					...
					<div class="wpcpf-new-faq">
						<select class="wpcpf-new-faq-type">
				...
				function product_faqs( $product_id ) {
					...
					if ( ! empty( $faqs ) ) {
						...
						foreach ( $faqs as $faq ) {
							...
							$content .= '<div class="wpcpf-faq-title">' . $faq['title'] . '</div>';
							$content .= '<div class="wpcpf-faq-content">' . $faq['content'] . '</div>';
				...
				function product_data_panels() {
					global $post;
					$post_id    = $post->ID;
					$saved_faqs = get_post_meta( $post_id, 'wpcpf_faqs', true );
					wp_enqueue_editor();
					?>
					<div id='wpcpf_settings' class='panel woocommerce_options_panel wpcpf_settings'>
						<div class="wpcpf-global">
							<a href="<?php echo admin_url( 'edit.php?post_type=wpc_product_faq' ); ?>" target="_blank"><?php esc_html_e( 'Manage Global FAQs', 'wpc-product-faqs' ); ?></a>
						</div>
						<div class="wpcpf-global">
							<?php echo sprintf( esc_html__( 'You can use the shortcode %s to show this product FAQs.', 'wpc-product-faqs' ), '<strong>[wpc_product_faqs id="' . esc_attr( $post_id ) . '"]</strong>' ); ?>
						</div>
						<div class="wpcpf-faqs">
							<?php if ( is_array( $saved_faqs ) && ( count( $saved_faqs ) > 0 ) ) {
								foreach ( $saved_faqs as $saved_faq ) {
									self::faq( $saved_faq );
								}
							} ?>
						</div>
